function [] = save_solution(cycle_data,my_filename,my_path)

% Print optimization problem output
file_name = fopen(fullfile(my_path,[my_filename,'_solution_optimization.txt']), 'w');
bounds_summary = cycle_data.optimization.bounds_summary;
...
fprintf(file_name,'|---------------------------------------------------------------------------------------------------------------|\n');
fprintf(file_name,'%31s %15s %12s %15s %12s %10s \n', header{1}, header{2}, header{3}, header{4}, header{5}, header{6});
for i = 1:size(name,2)
    fprintf(file_name,'%31s %15.4f %12.4f %15.4f %12s %10s \n', name{i}, value_min{i}, value{i}, value_max{i}, satisfied{i}, active{i});
end
fprintf(file_name,'|---------------------------------------------------------------------------------------------------------------|\n');

constraint_summary = cycle_data.optimization.constraint_summary;
...
fprintf(file_name,'|---------------------------------------------------------------------------------------------------------------|\n');
fprintf(file_name,'%31s %15s %12s %15s %10s %12s %10s \n', header{1}, header{2}, header{3}, header{4}, header{5}, header{6}, header{7});
for i = 1:size(name,2)
    fprintf(file_name,'%31s %15.4f %12.4f %15.4f %10s %12s %10s \n', name{i}, value_min{i}, value{i}, value_max{i}, applied{i}, satisfied{i}, active{i});
end
fprintf(file_name,'|---------------------------------------------------------------------------------------------------------------|\n');
fclose(file_name);
